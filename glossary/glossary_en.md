...
Intelligence refers to an agent's ability to achieve goals across a wide variety of environments. This definition by Legg and Hutter (2007) attempts to distill a common core from over 70 different definitions. The term is among the most conceptually contested in psychology and AI research. Despite over a century of scientific research, no generally accepted definition exists, and the term is used with sometimes contradictory meanings.
...
In AI research, intelligence is often operationally measured through [[#benchmark|benchmark]] performance, which has drawn criticism since high benchmark scores do not necessarily imply generalization capability. The debate about whether current AI systems are "intelligent" depends substantially on which definition is applied. This entry presents these various perspectives without making a final determination.
...
A bundle of competencies that enables individuals to critically evaluate AI technologies, communicate and collaborate effectively with AI, and deploy AI as a tool in various contexts. Long and Magerko (2020) coined the term in an influential CHI study and identified 17 core competencies, which they grouped into five clusters.

The concept encompasses four dimensions. **Foundational knowledge** refers to understanding what AI is and how it works, including the distinction between rule-based systems and machine learning, the role of training data, and the limitations of current systems. **Critical evaluation** denotes the ability to assess AI outputs for plausibility, [[#Bias]], and [[#Hallucinations (Confabulations)|confabulations]], as well as recognizing the limits of one's own judgment. **Ethics and attitude** encompasses reflection on societal implications, power asymmetries, and the question of when AI use is appropriate. **Practical application** refers to the ability to effectively use AI tools for one's own purposes, including [[#Prompt Engineering]] and integration into workflows.

AI Literacy differs from related concepts such as *Digital Literacy* (broader, encompassing all digital technologies) and *Data Literacy* (focused on data understanding and analysis). It is not a static competency but must evolve with technological development. The plural form *AI Literacies* emphasizes that it is a heterogeneous bundle of context-dependent skills, not a single, universal competency.

* Long, Duri, and Brian Magerko. "What is AI Literacy? Competencies and Design Considerations". _Proceedings of the 2020 CHI Conference on Human Factors in Computing Systems_, Association for Computing Machinery, 2020, 1–16. [https://doi.org/10.1145/3313831.3376727](https://doi.org/10.1145/3313831.3376727).
* Ng, Davy Tsz Kit, Jac Ka Lok Leung, Samuel Kai Wah Chu, and Maggie Shen Qiao. "Conceptualizing AI Literacy: An Exploratory Review". _Computers and Education: Artificial Intelligence_, Vol. 2, 2021. [https://doi.org/10.1016/j.caeai.2021.100041](https://doi.org/10.1016/j.caeai.2021.100041).
...
Language models with typically fewer than 10 billion [[#parameter|parameters]], optimized for efficiency and specialized tasks compared to larger [[#llm|LLMs]]. While LLMs function as generalists, SLMs can be characterized as specialists that achieve comparable or better performance at lower cost for focused tasks such as document classification, code routing, or summarization.

The advantages of SLMs lie in three areas. **Inference speed** results from fewer computations per prediction. **Cost efficiency** arises from lower GPU memory requirements and reduced operating costs. **Governance and privacy** are facilitated because SLMs can be operated on-premise without external API calls, which is relevant for regulated industries.

The boundary between SLM and LLM is not sharply defined and shifts with technological progress. Models that were considered large a few years ago now fall into the SLM category. The distinction is therefore less an absolute size category than a functional classification based on deployment profile.
...
Functionally, Frontier Models are distinguished by superior capabilities in complex [[#Reasoning]], multi-step planning, and [[#tool-use|Tool Use]]. They typically form the foundation for [[#Agentic AI|agentic capabilities]], as they can coherently maintain longer reasoning chains and orchestrate autonomous workflows. The distinction from [[#llm|LLMs]] and [[#slm|SLMs]] is primarily functional, not purely quantitative; a model is considered Frontier if it pushes performance boundaries in relevant [[#Benchmark|benchmarks]] at the time of its release.
...
Catastrophic Forgetting (also known as Catastrophic Interference) refers to the phenomenon where neural networks abruptly and almost completely lose previously acquired knowledge when trained on new tasks or data. The term was introduced in 1989 by Michael McCloskey and Neal J. Cohen, and further investigated by Roger Ratcliff in 1990.

The cause lies in how neural networks store knowledge. Unlike traditional databases that store information in separate memory units, neural networks distribute their knowledge across the weights of all connections in the network. When training on new tasks, these weights are overwritten by the optimization process, causing the configurations for earlier tasks to be lost.

An illustrative example from research history demonstrates the problem: McCloskey and Cohen trained a network first on simple addition problems with the number one (1+1, 1+2, and so on). When the network was subsequently taught addition problems with the number two, it lost the ability to solve the original problems, even though it had previously mastered them reliably.

The problem can be traced back to the Stability-Plasticity Dilemma, which Stephen Grossberg formulated as early as 1980. Stability describes the ability to preserve what has been learned, while plasticity describes the ability to absorb new information. Both properties compete with each other, as the mechanisms that enable remembering also make change more difficult. The optimal balance between these two poles remains an unsolved fundamental problem in AI research and is closely connected to [[#Continual Learning]].

* McCloskey, Michael and Neal J. Cohen. "Catastrophic Interference in Connectionist Networks: The Sequential Learning Problem". _Psychology of Learning and Motivation_ 24 (1989): 109–165. [https://doi.org/10.1016/S0079-7421(08)60536-8](https://doi.org/10.1016/S0079-7421(08)60536-8)
* Ratcliff, Roger. "Connectionist Models of Recognition Memory: Constraints Imposed by Learning and Forgetting Functions". _Psychological Review_ 97, No. 2 (1990): 285–308. [https://doi.org/10.1037/0033-295X.97.2.285](https://doi.org/10.1037/0033-295X.97.2.285)
...
Continual Learning (also known as Lifelong Learning or incremental learning) refers to the ability of a system to acquire new information or skills over time without losing previously acquired knowledge. For humans, this is self-evident: someone who learns to play guitar and later adds violin doesn't forget how to play guitar, even if some rustiness sets in. Neural networks behave fundamentally differently and are prone to [[#Catastrophic Forgetting]].

Research distinguishes three fundamental scenarios according to van de Ven et al. (2022): Task-incremental learning, where clearly distinguishable tasks are learned sequentially; Domain-incremental learning, where the same type of problem is learned in different contexts; and Class-incremental learning, where a model must learn to distinguish between an increasing number of classes. The latter is considered the most difficult scenario.

For practical application with large language models, a supplementary heuristic can be established: Context-based recall within a session is considered largely solved through large [[#Context Window|context windows]]. Cross-session recall uses external storage systems and [[#Retrieval Augmented Generation (RAG)]], though retrieval of relevant information does not work reliably. Task-specific adaptation through [[#Fine-Tuning]] often leads to loss of general capabilities. True Continual Learning in the narrower research sense means updating model weights in real-time without degradation of existing knowledge.

Two competing positions exist in the research community. Skeptics argue that the [[#Transformer]] architecture inherently has this limitation and scaling will not solve the problem—a new architectural paradigm is required. Pragmatists hold that system-level solutions such as extended context windows, better retrieval systems, and intelligent information disclosure can achieve functionally equivalent results without solving the algorithmic problem.

* van de Ven, Gido M., Tinne Tuytelaars and Andreas S. Tolias. "Three Types of Incremental Learning". _Nature Machine Intelligence_ 4 (2022): 1185–1197. [https://doi.org/10.1038/s42256-022-00568-3](https://doi.org/10.1038/s42256-022-00568-3)
* Parisi, German I., et al. "Continual Lifelong Learning with Neural Networks: A Review". _Neural Networks_ 113 (2019): 54–71. [https://doi.org/10.1016/j.neunet.2019.01.012](https://doi.org/10.1016/j.neunet.2019.01.012)
...
The capability for autonomous task execution correlates with model class. [[#frontier-model|Frontier Models]] typically exhibit the strongest agentic capabilities, as complex multi-step workflows require coherent reasoning over longer chains. However, [[#slm|SLMs]] can also operate agentically in constrained domains, provided task complexity remains limited.
...
However, parameter count is not the sole indicator of model performance. [[#Mixture of Experts (MoE)|MoE architectures]] can have significantly more parameters at the same inference speed, since only a fraction is activated per token. Additionally, training quality, data selection, and [[#Post-Training]] influence capabilities more than raw size. The blanket equation of "more parameters = better model" is therefore misleading, as demonstrated by the performance of modern [[#slm|SLMs]] on specialized tasks.
